...
ggplot(trendplotdf, aes(EV))+
  ...
  labs(x = "Év", y = "Lakásépítések tízezer lakosra(db)")+
  theme_minimal()+
...
```{r}
corrplot(cor(df[,c(4:8,14:16,19,38:40)], use = "complete.obs"), method = "color")
```
...
```{r}
lavaanPlot(model = fit, node_options = list(shape = "box", fontname = "Helvetica"), edge_options = list(color = "grey"), coefs = T, stars  = c("regress"))

save_png(lavaanPlot(model = fit, node_options = list(shape = "box", fontname = "Helvetica"), edge_options = list(color = "grey"), coefs = T, stars  = c("regress")), "SEMplot_sign.png")
```
